<?php
class Addify_Transfer_Reciver extends WC_Email {

	public function __construct() {
		$this->id             = 'Addify_transfer_reciver'; // Unique ID to Store Emails Settings
		$this->title          = __( 'Wallet transfer recipient', 'woo_addf_wm' ); // Title of email to show in Settings
	...
	public function get_default_subject() {
		return __( 'Transfer amount received notification', 'woo_addf_wm' );
	}
	// Step 4: change default heading of email by overriding the parent class method
	public function get_default_heading() {
		return __( "You've received a amount in your wallet", 'woo_addf_wm' );
	}
	 
	
	// Step 5: Must over ride trigger method to replace your placeholders and send email
	public function trigger( $user_name_for_email, $reciver_email, $sendr_email, $pay_amount ) {
		...
		if ( $this->is_enabled() ) {
			$this->recipient = $reciver_email; // Set the recipient email
...
if (class_exists('Addify_Transfer_Reciver')) {
	new Addify_Transfer_Reciver();
}
